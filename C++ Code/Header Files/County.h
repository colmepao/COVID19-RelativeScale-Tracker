...
public:
	...
	//Prints the average weekly total deaths/positive cases for a month range. In this project we are only looking at March-June as that was the constraint we defined; however, this function could be abstracted to a range between specific days rather than just months. Current functionality assumes a month is 29.5 days, which is the technical average month length of a year. So there will be more error for a smaller range of months.
	void PrintAvg(int startMonth, int endMonth, int endDay);
};
